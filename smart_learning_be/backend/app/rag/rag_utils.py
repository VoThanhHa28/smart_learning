...
def build_context(docs: List[Document], limit_chars: int = 5000) -> str:
    ...
    def safe_sort_key(d):
        ...
        sr = float(meta.get("semantic_score", 0.0))
    ...
    for i, d in enumerate(sorted_docs, start=1):
        meta = d.metadata or {}
        pg = meta.get("page")
        pg_tag = f"p{pg}" if isinstance(pg, int) else "p?"
        tag = f"[D{i}-{pg_tag}]"

        heading = meta.get("heading") or meta.get("headings", ["-"])
        heading_str = heading[0] if isinstance(heading, list) else heading
        src = meta.get("doc_source", "") or meta.get("source", "")
        content_type = meta.get("content_type", "text")
        section_pos = meta.get("section_position", "")
        diff = meta.get("difficulty", "")
        tags = ", ".join(meta.get("semantic_tags", []))

        title = " | ".join([x for x in [tag, heading_str, src] if x])
        header = f"### {title}\n(meta: type={content_type}; pos={section_pos}; diff={diff}; tags={tags})"
        text = (d.page_content or "").strip()

        block = f"{header}\n{text}\n"
        parts.append(block)

        # nguồn rút gọn cho bảng tra cứu
        index_lines.append(f"{tag} {src}".strip())
    ...
    # bảng tra cứu nguồn ở cuối context (giúp LLM bám đúng tag)
    if index_lines:
        context += "\n\n## SOURCES INDEX\n" + "\n".join(index_lines)
