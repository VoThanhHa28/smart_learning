...
from ...rag.retrieval.vectorstore import upsert_chunks
...
class DoclingPipeline:
    ...
    async def process_file(self, file_path: str, course_id: str = "default", subject: str = "general"):
        ...
        # 🧭 Detect TOC
        ocr_text = None
        if not is_text_based_pdf(file_path):
            try:
                ocr_text = call_ocr_service(file_path)  # đã có sẵn trong code bạn
            except Exception as e:
                logging.warning(f"⚠️ OCR fallback failed: {e}")

        toc_data = extract_toc_hybrid(file_path, ocr_text=ocr_text)
        logging.info(f"📖 TOC preview: {toc_data[:5]}")
