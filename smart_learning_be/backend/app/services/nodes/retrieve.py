...
from app.services.rag_state import debug_state
from app.services.hybrid import hybrid_retrieve
from app.services.rrf import rrf_merge
from app.services.rag_runtime import doc_sig, mark_doc, mmr_select
from app.services.config import CONCURRENCY, RRF_INTERQUERY_K, RRF_INTERQUERY_TOPN, SQ_MIN_PER_GROUP
...
async def retrieve(state: dict):
    ...
    for i, _ in enumerate(sq_items, 1):
        picked += (bags.get(f"sq::{i}", [])[:SQ_MIN_PER_GROUP])
